...
const SituationAnimalScreen = ({ navigation }) => {
    const { state, setSituation } = useContext(SortingContext)
    console.log(state.code);

    const handlesetSituation = (species) => {
        setSituation(species);
    };

    return (
        <SafeAreaView style={styles.container}>
            <Header hasReturn={false} code={ state.code } navigation={navigation} />
